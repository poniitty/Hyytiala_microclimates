...
# List Haxo files part 1
f <- list.files("data/hyytiala_2021", pattern = "HAXO-", full.names = T, recursive = T)

fi <- data.frame(file = f,
                 site = parse_number(unlist(lapply(f, function(x) strsplit(x, "/")[[1]][3]))))

f <- list.files("data/Hyytiälä21/HAXOT", pattern = "HYY_", full.names = T, recursive = T)

fi <- bind_rows(fi,
                data.frame(file = f,
                           site = parse_number(unlist(lapply(f, function(x) strsplit(x, "/")[[1]][4])))))

sum(duplicated(fi %>% select(site))) # Good if zero
...
fi %>% filter(!site %in% old_times$site)
old_times %>% filter(site %in% fi$site) -> old_times

old_times %>% filter(!complete.cases(.))
# old_times[old_times$site == 39, "min_time"] <- as_datetime("2019-09-14 23:59:00", tz = "Etc/GMT-2")

fi %>% filter(!site %in% unique(old_times$site)) # Good no new site id's
...
df2 %>% mutate(haxo_probl = ifelse(site == 10, 0, haxo_probl)) -> df2
df2 %>% mutate(haxo_probl = ifelse(site == 27, 0, haxo_probl)) -> df2
df2 %>% mutate(haxo_probl = ifelse(site == 47, 0, haxo_probl)) -> df2
